...
functions.http("helloWorld", async (req, res  ) => {
  ...
  // LINEのWebhookイベントでない場合、またはevents配列が空の場合は処理をスキップ
  if (!req.body || !Array.isArray(req.body.events) || req.body.events.length === 0) {
    console.log("INFO: Not a LINE webhook event or empty events array. Responding with 200 OK.");
    res.status(200).send("OK"); // ヘルスチェックなどには200 OKを返す
    return;
  }
  try {
